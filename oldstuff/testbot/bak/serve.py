...
class Sites():
    ...
    def parse(self, messageBody, phonenum):
        ...
        allSites = self.sites
        allEmployees = self.employees
        if keywords[0] in ['myalias', 'myname', 'forcemyalias', 'forcemyname']:
            if keywords[0].replace('force') in ['myalias']:
                pstr = 'MYALIAS'
                ppstr = 'alias'
                asknext = False
            elif keywords[0].replace('force') in ['myname']:
                pstr = 'MYNAME'
                ppstr = 'name'
                asknext = False
            elif keywords[0].replace('force') in ['mypicture']:
                pstr = 'MYPICTURE'
                ppstr = 'mypicture'
                asknext = True
            if len(keywords) == 1:
                if phonenum in allEmployees.keys() and ppstr in allEmployees[phonenum].keys():
                    return allEmployees[phonenum][ppstr]

            if len(keywords) != 2 and not asknext: 
                    return ('text', 'Invalid Command.  Usage: %s %s' % (keywords[0], pstr))

            if phonenum in allEmployees.keys() and ppstr in allEmployees[phonenum].keys() and not keywords[0].startswith('force') :
                return ('text', 'Unsuccessful. %s for %s already set to %s. To reset send FORCE%s %s' % (pstr, phonenum, keywords[1], pstr, ppstr))

            if asknext:
                self.stagetag[phonenum] = ('attach', '', ppstr)
                return ('text', 'Please send content for  %s ' % (ppstr))
            else:
                allEmployees[phonenum][ppstr] = ('text', keywords[1])
                return ('text', 'Successfully updated %s ' % (ppstr))
